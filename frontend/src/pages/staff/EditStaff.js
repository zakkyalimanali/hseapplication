...
export default function EditStaff() {
  ...
  useEffect(() => {
    axios.get(`${API_BASE}/hseapp/staff/${params.id}/`, {
      headers: { Authorization: `Bearer ${authTokens.access}` },
    }).then(res => {
      const d = res.data
      setName(d.name || '')
      setPosition(d.position || '')
      setStaffIdNumber(d.staff_id_number || '')
      setGender(d.gender || '')
      setDateOfBirth(d.date_of_birth || '')
      setJoiningDate(d.joining_date || '')
      setNationality(d.nationality || '')
      setCitizenship(d.citizenship || '')
      setHomeAddress(d.home_address || '')
      setTelephoneNumber(d.telephone_number || '')
      setEmailAddress(d.email_address || '')
      setSmartCardNumber(d.smart_card_number || '')
      setSmartCardColour(d.smart_card_colour || '')
      setPassportNumber(d.passport_number || '')
      setPassportExpiryDate(d.passport_expiry_date || '')
      setYearlyLeaveDays(d.yearly_leave_days ?? '')
      setYearlyLeaveTaken(d.yearly_leave_taken ?? '')
    }).catch(console.log)
  }, [params.id])

  const onUpdate = (e) => {
    e.preventDefault()
    StaffAPI.patch(`/${params.id}/`, {
  ...
  return (
    <div style={{ padding: '40px', maxWidth: '860px', margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: '32px' }}>
        ...
        <h2 style={{ color: NAVY, fontWeight: '800', margin: '0 0 4px' }}>Edit Staff</h2>
